...
% Note: Pulser uses counter0 to time digital i/o. The other counters are
% free to use.
% TODO: Should add a save/load funtion.

%% Hardware Configuration  
pulser.sampleRate = 20000;         % In Hz applies to all tasks for now, but you could (theoretically) create a rate for each task.
pulser.acqTime=360;					% Total acquisition time for a trial.
...
pulser.AOChans = 0; 				% Numerical row vector.
pulser.AOChanLabels = {'Whisker Stim'};  % Optional Labels
...
% Note: This is set up to take the arguments needed for pulse train, or ramp construction.  Analog and digital are pulserured seperatly. 
...
pulser.trainTypes=[1];			% 1 for pulse-train, 2 for ramp
pulser.trainAmplitudes=[-8];		% Peak amplitude your output will reach. Pulse height, ramp peak etc. (In Volts)		
pulser.rampSpeeds=[0];			% Speed (V/sec) at which your ramp will climb from baseline to peak.
pulser.baselineValues=[0];		% Baseline from which to operate around. Note you can have a dc input by calling a pulse train of any type with no amplitude.
pulser.baselineTimes=[60];		% Time to hold at baseline before any patterned output starts.
pulser.numTrains=[4];			% How many times do you want your train or ramp to repeat?
pulser.interTrainInterval=[60];	% Time between train (or ramp, or whatever) repetitions.
pulser.pulseWidths=[.010];		% How wide should your pulses be? (in seconds)
pulser.numPulses=[15];			% How many pulses in a given repetition will you give?
pulser.pulseRate=[10];		% How much time in between your pulses.  TODO: I should have a toggle that lets the user put this is rate or time.
...
pulser.pulseDigFrequency=[20];	% How much time in between your pulses.  TODO: I should have a toggle that lets the user put this is rate or time.		

% Shutters
pulser.shutterLines=[7 6];
...
pulser.shutterToggles=[1 1];  %What to write 1: High 0: Low
